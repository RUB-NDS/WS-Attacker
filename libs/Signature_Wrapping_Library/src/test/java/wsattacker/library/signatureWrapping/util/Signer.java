...
//        valContext.setURIDereferencer(valContext.getURIDereferencer());
        ...
        if (!allvalid) {
            ...
            for (int j = 0; itRef.hasNext(); j++) { // j is only used if we calculate the Reference Digest, which is out-commmented for performance issues...
