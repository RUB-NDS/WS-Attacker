...
public class SignatureManager {
     ...
     * List<Element> securityList = findChildren(header, "Security",
     * URI_NS_WSSE_1_0);
     ...
     * List<Element> assertionList = findChildren(securityElement, "Assertion",
     * null); // TODO: Use SAML Namespace?
     * for (Element assertion : assertionList) {
     * signatureParentList.add(assertion);
     * }
     *
     * for (Element signatureParent : signatureParentList) {
     * List<Element> signatureList = findChildren(signatureParent, "Signature",
     * XMLSignature.XMLNS);
     * for (Element signature : signatureList) {
     ...
     * original Signed Content and additional contains the Payload to use for
     * the
     * attack.
     *
