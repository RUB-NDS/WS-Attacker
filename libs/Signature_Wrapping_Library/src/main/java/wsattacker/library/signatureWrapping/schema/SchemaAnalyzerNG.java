...
public class SchemaAnalyzerNG implements SchemaAnalyzerInterface {

    private static Logger log = Logger.getLogger(SchemaAnalyzerNG.class);
    Document analyzingDocument, expandedAnalyzingDocument;
    Map<String, Document> schemaMap;
    List<QName> filterList;

    public SchemaAnalyzerNG() {
    ...
    /**
     * Checks theElement has an ancestor element with given ancestorLocalName
     * and ancestorNamespaceURI.
     *
     * @param theElement
    ...
    public void createChildElementForElement(Element theElement, String localName, String namespaceURI, String fallbackPrefix) {
        ...
        theElement.appendChild(
          theElement.getOwnerDocument().createElementNS(
          theElement.getNamespaceURI(), prefix + localName));
    }
    ...
    public List<Element> findPossibleChildElements(String namespaceURI, String localName) {
        ...
        if (complexType == null) {
            log.info(String.format("Element '%s:%s' seems to not to have a complex declaration.", namespaceURI, localName));
            return new ArrayList<Element>();
        }
    ...
    private Element findXInSchema(String x, String namespaceURI, String localName) {
        ...
        String xpath;
        xpath = "//*[local-name()='" + x + "' and namespace-uri()='" + URI_NS_SCHEMA + "' and @name='" + localName + "']";
        ...
        if (complexTypeList.size() == 0) {
            ...
            log.warn(String.format("More than one possible schema definition found for %s:%s", namespaceURI, localName));
        }
    ...
    public Element findComplexTypeForElement(Element elementSchema) {
        ...
        if (type.isEmpty()) {
            ...
            List<Element> complexChildElementList = DomUtilities.findChildren(elementSchema, "complexType", URI_NS_SCHEMA, true);
