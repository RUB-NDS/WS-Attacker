...
public class SchemaAnalyzer implements SchemaAnalyzerInterface {
     ...
     * (non-Javadoc)
     * @see
     * wsattacker.plugin.signatureWrapping.schema.SchemaAnalyserInterface#setFilterList(java.util.List)
     */
    @Override
     ...
     * (non-Javadoc)
     * @see
     * wsattacker.plugin.signatureWrapping.schema.SchemaAnalyserInterface#appendSchema(org.w3c.dom.Document)
     */
    @Override
...
// (DomUtilities.domToString(analyzingDocument).equals(DomUtilities.domToString(n.getOwnerDocument()))));
    ...
    private void findExpansionPoint(Set<AnyElementPropertiesInterface> result,
      Element start) {
        ...
        if (complexType == null) {
            log.trace("\tNo ComplexType can be found.. maybe simple type -> Aborting this Element");
//			return;
        } else {
            ...
            for (Element elementSchema : allowedChildElements) {
                // Get Element Localname
                String localName = elementSchema.getAttribute("ref").substring(1 + elementSchema.getAttribute("ref").indexOf(':'));
                if (localName.isEmpty()) {
                    continue;
                }
                log.trace("\tAllowed Child: '" + localName + "'");
                // Get Namespace
                String[] prefixNamespacePair = getTargetNamespace(elementSchema);
                ...
                if (DomUtilities.findChildren(start, localName, prefixNamespacePair[1], false).isEmpty()) {
                    ...
                    if (contained) {
                        ...
                        if (start.getPrefix() != null) {
                            start.appendChild(start.getOwnerDocument().createElementNS(start.getNamespaceURI(), start.getPrefix() + ":" + localName));

                        } else {
                            start.appendChild(start.getOwnerDocument()
                              .createElementNS(start.getNamespaceURI(), localName));
                        }
    ...
    /**
     * Returns the Schemas Complex Type of the xs:any up node E.g. xs:any ->
     * soap:Header of the xs:any in the Header.
     *
     * @param xsAny
     *
     * @return
...
// System.out.println("XPATH to Eval:\n"+xpath+"\n");
    ...
    public List<String> getContainedSchemaNamespaces() {
        List<String> ret = new ArrayList<String>();
        try {
            List<? extends Node> match = DomUtilities.evaluateXPath(schema, "//attribute::*[local-name()='targetNamespace']");
            for (Node n : match) {
                ret.add(n.getNodeValue());
            }
        } catch (XPathExpressionException ex) {
            java.util.logging.Logger.getLogger(SchemaAnalyzer.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        return ret;
    }
}
