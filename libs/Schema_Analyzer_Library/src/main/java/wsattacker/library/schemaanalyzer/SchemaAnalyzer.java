 ...
 * 2) A Null Schema Analyzer, which allows any element to have any child
 * element.
 */
public interface SchemaAnalyzer {
    ...
    /**
     * Find an expansion point in the Schema starting with Node fromHere. The
     * returned Elements are not part of the old
     * Document. Instead, a new Document is used where each possible but not
     * occuring element is added. So, the original
     * Document will not be modified! Elements in the filterList will not be
     * searched.
     *
     * @param fromHere
     * @param filterList
