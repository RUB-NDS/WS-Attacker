 ...
 * @author Dennis
 */
public class EncryptionAttributeIdWeaknessTest
{
    ...
    public EncryptionAttributeIdWeaknessTest()
    {
        ...
        InvalidWeaknessException
    {
        ...
        EncryptionAttributeIdWeakness encAttrWeak =
            (EncryptionAttributeIdWeakness) FactoryWeakness.generateWeakness( WeaknessType.ATTR_ID_WEAKNESS, encEl,
                                                                              encKey );
        assertEquals( WrapModeEnum.ENCKEY_WRAP_ENCDATA, encAttrWeak.getWrapMode() );

        assertEquals( 4, encAttrWeak.getPossibleNumWeaks() ); // 2 (keyRef append
        // only, delete +
        // append) x 1.5
        // (change ID ->
        // keyrefWeak, keep
        ...
        // abuse the weakness
        for ( int i = 0; i < encAttrWeak.getPossibleNumWeaks(); ++i )
        {
            ...
            encAttrWeak.abuseWeakness( i, encKey.getEncryptedElement(), copyPayload );
        ...
        assertEquals( 2, newIdCounts );
        assertEquals( 2, newKeyRefCounts ); // append + delete all
        assertEquals( 3, newKeyRefURI ); // delete append, delete all, append new

    }

    // TODO: (signed) EncKey + (signed) EncData
    @Test
    public void testSignedEncKeyEncDataSigned()
        throws FileNotFoundException, XPathExpressionException, SAXException, IOException, InvalidPayloadException,
        InvalidWeaknessException
    {
        /*
         ...
         * assertEquals(2,m_SigManager.getPayloads().size()); assertEquals("Body"
         ...
         * EncryptedKeyElement encKey = encInfo.getEncryptedKeyElements().get(0); AbstractEncryptionElement encEl =
         * ((DataReferenceElement)encInfo.getEncryptedKeyElements ().get(0).getReferenceElementList().get(0))
         * .getRefEncData(); EncryptionSchemaWeakness encDataSchemWeak = (EncryptionSchemaWeakness)
         * FactoryWeakness.generateWeakness(WeaknessType.SCHEMA_WEAKNESS, encEl,encKey); EncryptionSchemaWeakness
         * encKeySchemWeak = (EncryptionSchemaWeakness) FactoryWeakness.generateWeakness(WeaknessType.SCHEMA_WEAKNESS,
         * encKey,encKey); assertEquals(EncSignedModeEnum.SIGN_ENCKEY_SIGN_ENCDATA,
         * encDataSchemWeak.getEncSignedMode()); assertEquals(EncSignedModeEnum.SIGN_ENCKEY_SIGN_ENCDATA,
         * encKeySchemWeak.getEncSignedMode()); List<QName> filterList = new ArrayList<QName>(); filterList.add(new
         * QName(URI_NS_ENC, "EncryptedData")); filterList.add(new QName(URI_NS_DS, "SignedInfo")); filterList.add(new
         * QName(URI_NS_DS, "SignatureValue")); filterList.add(new QName(m_SigManager.getPayloads(
         * ).get(0).getSignedElement().getNamespaceURI(), m_SigManager.getPayloads
         * ().get(0).getSignedElement().getLocalName())); filterList.add(new QName
         * (m_SigManager.getPayloads().get(1).getSignedElement().getNamespaceURI (),
         * m_SigManager.getPayloads().get(1).getSignedElement().getLocalName( )));
         * m_SchemaAnalyzer.setFilterList(filterList); encDataSchemWeak.findSchemaWeakness(m_SchemaAnalyzer);
         * encKeySchemWeak.findSchemaWeakness(m_SchemaAnalyzer); // 54 schemaweaknesses
         * assertEquals((54*4),encDataSchemWeak.getPossibleNumWeaks()); // 2 (keyRef append only, delete all + append) x
         * 2 (change ID -> keyrefWeak, keep id -> keyRefWeak)
         * assertEquals((54*2),encKeySchemWeak.getPossibleNumWeaks()); int newIdCounts = 0; int newKeyRefCounts = 0; //
         * abuse the weakness /*for (int i = 0; i < encDataSchemWeak.getPossibleNumWeaks(); ++i) { Document copyDoc =
        ...
        InvalidWeaknessException
    {
        ...
        EncryptionAttributeIdWeakness encAttrWeak =
            (EncryptionAttributeIdWeakness) FactoryWeakness.generateWeakness( WeaknessType.ATTR_ID_WEAKNESS, encEl,
                                                                              null );
        assertEquals( WrapModeEnum.WRAP_ENC_ELEMENT, encAttrWeak.getWrapMode() );

        assertEquals( 2, encAttrWeak.getPossibleNumWeaks() ); // (change ID, keep
        // id )
        int newIdCounts = 0;
        int newKeyRefCounts = 0;
        int newKeyRefURI = 0;
        ...
        for ( int i = 0; i < encAttrWeak.getPossibleNumWeaks(); ++i )
        {
            ...
            encAttrWeak.abuseWeakness( i, attackProps.getSignedPart(), copyPayload );
            log.trace( "### " + i + ")\n" + domToString( copyDoc, true ) + "\n" );
            ...
            List<Element> encKeyInEncDataMatched =
                DomUtilities.findChildren( copyDoc, "EncryptedKey", URI_NS_ENC, true );
            assertEquals( 1, encKeyInEncDataMatched.size() );
        ...
        InvalidWeaknessException
    {
        ...
        assertEquals( 0, encAttrWeak.getPossibleNumWeaks() );
        ...
        InvalidWeaknessException
    {
        ...
        EncryptionAttributeIdWeakness encKeySchemWeak =
            (EncryptionAttributeIdWeakness) FactoryWeakness.generateWeakness( WeaknessType.ATTR_ID_WEAKNESS, encKey,
                                                                              encKey );
        assertEquals( WrapModeEnum.WRAP_ENCKEY_ENCDATA, encKeySchemWeak.getWrapMode() );

        assertEquals( 2 * 2, encKeySchemWeak.getPossibleNumWeaks() ); // (delete
        // all
        // keyrefs +
        // keep
        // refs)*2(changeID,
        ...
        for ( int i = 0; i < encKeySchemWeak.getPossibleNumWeaks(); ++i )
        {
            ...
            assertEquals( 1, matchedEncKey.size() );
