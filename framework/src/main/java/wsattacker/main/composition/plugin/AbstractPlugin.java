    ...
    implements SuccessInterface, Comparable<AbstractPlugin>, Serializable
{
    ...
    /**
     * * This method will be called if a new project ist started and will also be called first in startAttack() E.g. for
     * simplest implementation: setCurrentPoints(0)
     */
    ...
    /**
     * This method will be called after the attack thread is killed. The plugin can try to clean up everything, what
     * could not be finished in the abortHook(). The default implementation does nothing.
     */
    ...
    /**
     * * This method will be used to restore saved plugin configuration. The current implementation will only work for
        ...
        for ( AbstractOption savedOption : plugin.getPluginOptions() )
        {
            ...
            if ( currentOption != null )
            {
                if ( currentOption.isValid( savedValue ) )
                {
                    ...
                    Logger.getLogger( getClass() ).info( String.format( "Restored option %s=%s", savedOption.getName(),
                                                                        savedOption.getValueAsString() ) );
                }
                else
                {
                    Logger.getLogger( getClass() ).info( "Could not restore Value '" + savedValue + "' for Option '"
                                                             + savedOption.getName() + "' - The value is not valid!" );
                }
            }
            else
            {
                Logger.getLogger( getClass() ).error( "The Option '"
                                                          + savedOption.getName()
                                                          + "' is not present in this plugin. Contact the plugin author." );
            }
        }
    }
