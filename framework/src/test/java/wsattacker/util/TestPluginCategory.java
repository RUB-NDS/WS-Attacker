...
public class TestPluginCategory
{
        ...
        throws Exception
    {
        ...
        c1.addLeaf( new NullPlugin( "c1_1" ) );
        c1.addLeaf( new NullPlugin( "c1_2" ) );

        c2 = new PluginCategory( "B" );
        c2.addLeaf( new NullPlugin( "c2_2" ) );
        c2.addLeaf( new NullPlugin( "c2_1" ) );

        c3 = new PluginCategory( "A" );
        c3.addLeaf( new NullPlugin( "c3_1" ) );

        c4 = new PluginCategory( "B" );
        c4.addLeaf( new NullPlugin( "c4_1" ) );
        c4.addLeaf( new NullPlugin( "c4_2" ) );
        c4.addCategory( new PluginCategory( "S1" ) );
        c4.addCategory( new PluginCategory( "S2" ) );
    }

    private void testOneCategory( Category<String, AbstractPlugin> category, String expectedName, int expectedSubs,
                                  int expectedLeafs )
    {
        String categoryName = category.getName();
        int categorySubs = category.getSubCategorys().size();
        int categoryLeafs = category.getLeafs().size();
        assertTrue( String.format( "Name of '%s' should be %s", categoryName, expectedName ),
                    categoryName.equals( expectedName ) );
        assertTrue( String.format( "Category '%s': expected %d sub categorys, but got %d", categoryName, expectedSubs,
                                   categorySubs ), categorySubs == expectedSubs );
        assertTrue( String.format( "Category '%s': expected %d leafs, but got %d", categoryName, expectedLeafs,
                                   categoryLeafs ), categoryLeafs == expectedLeafs );
        assertTrue( String.format( "Category '%s': expected %d sub categorys, but got %d", categoryName, expectedSubs
                        + expectedLeafs, categorySubs + categoryLeafs ),
                    ( categorySubs + categoryLeafs ) == ( expectedLeafs + expectedSubs ) );
    }
    ...
    private void testAllLeafs( Category<String, AbstractPlugin> category, String... leaf )
    {
        ...
        assertTrue( String.format( "Number of leafs does not match. Expected %d, got %d", expectedLength, gotLength ),
                    expectedLength == gotLength );
    ...
    private void testAllSubs( Category<String, AbstractPlugin> category, String... sub )
    {
        ...
        for ( int i = 0; i < gotLength; ++i )
        {
            ...
            assertTrue( String.format( "Sub category %d does not match, %s != %s", i, got, expected ),
                        expected.equals( got ) );
        }
    }
