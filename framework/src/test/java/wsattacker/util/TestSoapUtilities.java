...
public class TestSoapUtilities
{
    private static final String MESSAGE = ""
        ...
        + "    <ns1:hc></ns1:hc>" + "    <ns1:double/>" + "   </soapenv:Header>"
        + "   <soapenv:Body xmlns:ns2=\"http://ns2.com\">" + "      <ns1:ServiceOperation>"
        + "         <ns1:ba>Body A</ns1:ba>"
        + "         <ns1:c><ns2:d><ns3:e xmlns:ns3=\"http://ns3.com\">Body D</ns3:e></ns2:d></ns1:c>"
        + "         <a><b/></a>" + "      </ns1:ServiceOperation>" + "   </soapenv:Body>" + "</soapenv:Envelope>";
        ...
        throws Exception
    {
        ...
        assertTrue( String.format( "Wrong Child (%s != %s)", childs.get( 0 ).getNodeName(), childname ),
                    childs.get( 0 ).getNodeName().equals( childname ) );
        ...
        for ( int i = 0; i <= 1; ++i )
        {
            assertTrue( String.format( "Wrong Child (%s != %s)", childs.get( i ).getNodeName(), childname ),
                        childs.get( i ).getNodeName().equals( childname ) );
        }
        ...
        throws Exception
    {
        ...
        String[] values =
            { "http://schemas.xmlsoap.org/soap/envelope/", "http://sn1.com", "http://ns2.com", "http://ns3.com" };
        for ( String key : keys )
        {
        ...
        for ( int i = 0; i < keys.length; ++i )
        {
            ...
            String uri = values[i];
            assertTrue( String.format( "Wrong Namspace, shoult be %s=%s", prefix, uri ),
                        namespaces.get( prefix ).equals( uri ) );
        }
    }

    @Test
    public void testGetInputNeeded()
        throws Exception
    {
        String msg = new String( MESSAGE );
        SOAPMessage soap = stringToSoap( msg );
        ...
        assertFalse( String.format( "Found more than possible, Expected %d, Found %d", shouldBeFound,
                                    inputNeeded.size() ), shouldBeFound > inputNeeded.size() );
        ...
        throws Exception
    {
        ...
        SOAPMessage soap = stringToSoap( msg );
        SOAPElement ele =
            getSoapChilds( getSoapChilds( getSoapChilds( soap.getSOAPBody() ).get( 0 ) ).get( 1 ) ).get( 0 );
        ...
        for ( int i = 0; i < testParents.size(); ++i )
        {
            ...
            assertTrue( String.format( "Wrong Parent, got %s, but expected %s", testName, trueName ),
                        trueName.equals( testName ) );
        }
    }
