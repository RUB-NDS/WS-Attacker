...
// (DomUtilities.domToString(analyzingDocument).equals(DomUtilities.domToString(n.getOwnerDocument()))));
                                  ...
                                  Element start)
  {
    ...
    for (Element ele : allowedChildElements)
    {
      ...
      if (DomUtilities.findChildren(start, localName, start.getNamespaceURI(), false).size() == 0)
      {
        // Create if not
  ...
  /**
   * Returns the Schemas Complex Type of the xs:any parent node E.g. xs:any -> soap:Header of the xs:any in the Header.
   * 
   * @param xsAny
   * @return
   */
