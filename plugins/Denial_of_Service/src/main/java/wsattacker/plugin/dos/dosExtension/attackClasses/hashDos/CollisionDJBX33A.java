...
public class CollisionDJBX33A implements CollisionInterface {
	...
	int hash = 5381;
	for (int i = 0; i < n; i++) {
	    //System.out.println(i +" out of "+n+" mit Byte: "+(int)byteArray[i]+" und Hash preCalc: "+ ((hash<<5)+hash)+" - HashFinal: "+(((hash<<5)+hash)+(int)byteArray[i]));
    ...
    /**
     * generates n Collisions for DJBX31A and writes result to sb
     * All collisions are written in XML-Attribute Style
     *
	...
	try {
	    ...
	    // calculate Exponent so that we can generate numberAttributes Elements
	    int exponent = 1;
	    ...
	    System.out.println("Runtime using DJBX31A creating n=" + numberAttributes + " collisions: " + d + "seconds");
    ...
    /**
     * get i-th ternaryCollisionString out of 2^n ternaryCollisionStrings since
     * 3^n Elemnts can be generated result string will have a length of 2*n
     * chars 2*n since each Element is represented by 2 char string!
     *
     * How does it work for n = 2 -> 2^2 = 9 possible strings?
     * Algorithm generates 2*9 Matrix and replaces numbers with collision strings
     * aka table with ternary-Numbers
     * row0: 00
     * row1: 01
     * row2: 10
     * row3: 11
     * 
     * for i = 2 -> 1th row is picked and created!
     * 
