    ...
    implements CollisionInterface
{
    ...
    public int getHash( String s )
    {
        ...
        for ( int i = 0; i < n; i++ )
        {
            ...
            // (33*hash) +" - HashFinal: "+((33*hash)+(int)byteArray[i]));
    ...
    public int hashBack( String s, int target )
    {
        ...
        for ( int i = n; i > 0; i-- )
        {
            ...
            // byteArray[i - 1] + " - HashFinal: " + (hash ^ (int) byteArray[i -
            // 1]) * 1041204193);
            // hash = ((hash<<5)+hash)^(int)byteArray[i];
            hash = ( ( hash ^ byteArray[i - 1] ) * 1041204193 );
        }
        // System.out.println(hash);
        return hash;
    }
    ...
    public int hashForth( String s )
    {
        ...
        {
            // System.out.println(i + " out of " + n + " mit Byte: " + (int)
            // byteArray[i] + " und Hash preCalc: " + (33 * hash) +
            // " - HashFinal: " + ((33 * hash) + (int) byteArray[i]));
            hash = ( ( hash << 5 ) + hash ) ^ byteArray[i];
        }
        // System.out.println(hash);
        return hash;
    ...
    /**
     * For this Hash Algrorithm it actually reads values from precomputed table due to lengthy collsion computation
     */
    @Override
    ...
    /**
     * Not really required for DJBX33X since we are using precomputed values and read everything directly from file!
     * 
    ...
    public int generateCollionsMeetInTheMiddle( long numberCollisions )
    {
        ...
        try
        {
            ...
            // - will result in 59444 unique entries (others are already
            // collisions)
            ...
            while ( numberOfFoundCollisions < numberCollisions )
            {
                ...
                if ( s3SuffixMatch != null )
                {
                    ...
                    if ( !s7Prefix.matches( "^[0-9]+[0-9a-zA-Z]*$" ) )
                    {
                        ...
                        {
                            // save result to Hashtable - so we can check that
                            // not in twice
                    ...
                    {
                        // System.out.println("Attribute Starts with number: "+
                        // s7Prefix);
                    }
                }
