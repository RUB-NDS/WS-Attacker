...
/**
 * After Attack Button is pressed, this Thread is started and RUNS IN LOOP!..
 * It continuously takes an UpdateClock-Object and puts it in the GUI-EventQueue..
 ...
 * 
 * Has to be this way -> pages S.216 ff. -> Thread NEVER updates GUI (or model???) itself!!
 * @author af
 *
 */
public class TickerThread extends Thread {  // implements Oberservable Subject
	...
	public TickerThread(AttackModel model){
		...
		// macht nix anderes als einmal Clock update auszuführen und dann fertig..
		this.model = model;
	...
	public void run(){
		try{
			while(!isInterrupted()){
				// Warum kann ich nicht hier direkt ModelMethode aufrufen und so GUI-Update provozieren?
				// -> einfach BadPractice?! kann zu Problemen führen S. 216
				// -> Gibt daher extra diese EventQueue = EDT, dort werden alle GUI-Änderungswünsche sequentiell abgearbeitet!!
